...
function AutomaticShutdown {
    ...
    $button_logic = {
    Start-Process powershell.exe -ArgumentList "-ExecutionPolicy Bypass -windowstyle hidden -file $ENV:ProgramData\CloudGaming\Automatic-Shutdown.ps1"
    $form1.Close()
    }
...
function idle {
$readfile = (Get-Content -Path $env:ProgramData\CloudGaming\Autoshutdown.txt) - 10
do {
